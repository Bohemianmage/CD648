...
export function Navbar({ onOpenReserva }) {
  ...
  setMenuOpen(false); // 🔄 Cierra menú móvil al seleccionar idioma
};;
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            >
              <button className="transition-transform hover:scale-105 flex items-center gap-1">
                {t('navbar.about')}
                <ChevronDown
                  className={`w-4 h-4 mt-[1px] transition-transform duration-200 ${aboutDropdownOpen ? 'rotate-180' : ''}`}
                />
              </button>
              <div
                className={`absolute left-0 mt-3 bg-white border border-gray-200 rounded-lg shadow-xl z-50 min-w-[180px] py-2 text-sm transition-all duration-200 ease-out transform ${aboutDropdownOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}
              >
          ...
          <div className="flex flex-col gap-2 w-full px-4 items-center">
            <div className="w-full max-w-xs">
              ...
              >
                ...
                <ChevronDown
                  className={`w-4 h-4 transition-transform duration-200 ${aboutMobileOpen ? 'rotate-180' : ''}`}
                />
              </button>

              {aboutMobileOpen && (
                <div className="mt-1 flex flex-col gap-1 items-center border border-gray-200 rounded-md bg-gray-50 px-2 py-2 shadow-sm">
                  <button
